...
namespace API_ASISTENCIA_ESTUDIANTIL.Controllers
{
    ...
    public class Tbl_Datos_PersonalesController : ControllerBase
    {
        ...
        {
            var _resultado = _CNDatos_Personales.CN_Listar_Datos_Personales();
        ...
        public ActionResult<List<CE_DATOS_PERSONALES>> FiltrarDatosPersonalesID([FromHeader] int? Id_Persona)
        {
            ...
            var _resultado = _CNDatos_Personales.CN_Filtrar_Datos_PersonalesID(_CEDatos_Personales);
            if (_resultado.Count == 0)
            {
                return NoContent();
            }
        ...
        public ActionResult<List<CE_DATOS_PERSONALES>> FiltrarDatosPersonales([FromHeader] string? DNI, string? nombre)
        {

            _CEDatos_Personales.DNI = DNI;
            _CEDatos_Personales.Primer_Nombre = nombre;

            var _resultado = _CNDatos_Personales.CN_Filtrar_Datos_Personales(_CEDatos_Personales);
            if (_resultado.Count == 0)
            {
                return NoContent();
            }
        ...
        public ActionResult InsertarDatosPersonales([FromBody] ApiModels.AM_DATOS_PERSONALES _Am_Datos_Personales)
        {
            ...
            try
            {
                _CNDatos_Personales.CN_Insertar_Datos_Personales(_CEDatos_Personales, out resultado, out mensaje);
