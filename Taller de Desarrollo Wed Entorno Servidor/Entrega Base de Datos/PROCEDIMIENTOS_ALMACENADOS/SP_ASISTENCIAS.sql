...
BEGIN
    ...
    ELSE IF (@Accion = 'FIL')
    BEGIN
        IF ISNULL(@Id_Estudiante_Grupo, ' ') = ' '  or isnull(@Id_Grupo_Asignatura, ' ') = ' '
		or isnull(@Fecha, ' ') = ' ' or isnull(@Asistio, ' ') = ' '
        BEGIN
            SET @O_Numero = -1
            SET @O_Msg = 'NINGUNO DE LOS PARAMETROS PUEDEN SER NULO, INGRESE AL MENOS UNO PARA FILTRAR'
        END
        ELSE
        BEGIN
            SELECT *
			...
			Fecha = @Fecha OR 
			Asistio LIKE '%' + @Asistio + '%'
            
            SET @O_Numero = 0
            SET @O_Msg = 'Filtrado Exitoso'
        END
    END
	ELSE IF(@Accion = 'GET')
BEGIN
...
EXEC SP_ASISTENCIAS 
    @Id_Grupo_Asignatura = 2,  -- O un valor que no existe
    @Accion = 'FIL',
