...
BEGIN
	...
	ELSE IF(@Accion = 'FIL')
		BEGIN
			IF ISNULL(@DNI, '') = '' AND ISNULL(@Primer_Apellido, ' ') = ' '
				BEGIN
					SET @O_Numero = -1
					SET @O_Msg = 'PARAMETRO DNI O PRIMER APELLIDO NO PUEDE SER NULO O VACIO'
		END
			ELSE
				BEGIN
					SELECT *
...
EXEC SP_DATOS_PERSONALES 
    @Primer_Nombre = 'Juan',
    @Segundo_Nombre = 'Pablo',
    @Primer_Apellido = 'García',
    @Segundo_Apellido = 'López',
    @Edad = '25',
    @Tipo_Cargo = 1, -- Asumiendo que 1 es un valor válido para Tipo_Cargo
    @Tipo_DNI = 1,   -- Asumiendo que 1 es un valor válido para Tipo_DNI
    @DNI = '12345678',
    @Genero = 1,     -- Asumiendo que 1 es un valor válido para Género
    @Nacionalidad = 1, -- Asumiendo que 1 es un valor válido para Nacionalidad
    @Departamento = 1,  -- Asumiendo que 1 es un valor válido para Departamento
    @Estado_Civil = 1,  -- Asumiendo que 1 es un valor válido para Estado Civil
    @Id_Creador = 1,    -- Asumiendo que 1 es el ID del creador
    @Accion = 'INS',
    @O_Numero = @O_Numero OUTPUT,
    @O_Msg = @O_Msg OUTPUT;

SELECT @O_Numero AS NumeroResultado, @O_Msg AS MensajeResultado;
