...
export default function FiltrarIDAsignatura() {
  ...
  const handleBuscar = async () => {
    ...
    const requestOptions = {
      method: 'GET',
      headers: {
        // La clave es enviar el valor de la búsqueda en la cabecera 'Id_Asignatura'
        'Id_Asignatura': busqueda
      }
    };

    try {
      // La URL ya no necesita el parámetro ?busqueda=...
      const res = await fetch(`${appsettings.apiUrl}Cls_Asignaturas/BUSCAR_ASIGNATURAS_POR_ID`, requestOptions);
    ...
    <Container className="mt-5">
      <Row>
        <Col sm={{ size: 10, offset: 1 }}>
          <h4>Filtrar asignatura por ID</h4>
          <Input
            type="text"
