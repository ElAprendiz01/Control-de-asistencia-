import React, { useState } from 'react';
import '../../styles/iinserActu.css';

const ActualizarTipoCatalogo = () => {
  const [id, setId] = useState('');
  const [nombre, setNombre] = useState('');
  const [activo, setActivo] = useState(false);
  const [mensaje, setMensaje] = useState('');

  const handleSubmit = async (e) => {
    e.preventDefault();

    const token = localStorage.getItem('token');
    if (!token) {
      setMensaje('⚠️ No se encontró el token. Iniciá sesión nuevamente.');
      return;
    }

    const data = {
      Id_Tipo_Catalogo: parseInt(id),
      Nombre: nombre,
      Activo: activo
    };

    try {
      const response = await fetch('https://localhost:44357/api/TIPO_CATALOGO/Actualizar_tipo_catalogo', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify(data)
      });

      if (response.ok) {
        setMensaje('✅ Tipo catálogo actualizado correctamente');
        setId('');
        setNombre('');
        setActivo(false);
      } else {
        const errorData = await response.json();
        setMensaje(`❌ Error: ${errorData.mensaje || 'No se pudo actualizar'}`);
      }
    } catch (error) {
      setMensaje('⚠️ Error de conexión con el servidor');
    }
  };

  return (
    <div className="formulario-galactico">
      <h2>📁 Actualizar Tipo Catálogo</h2>
      <form onSubmit={handleSubmit}>
        <div className="campo">
          <label>ID:</label>
          <input
            type="number"
            value={id}
            onChange={e => setId(e.target.value)}
            placeholder="ID del tipo catálogo"
            required
          />
        </div>
        <div className="campo">
          <label>Nombre:</label>
          <input
            type="text"
            value={nombre}
            onChange={e => setNombre(e.target.value)}
            placeholder="Nombre del tipo catálogo"
            required
          />
        </div>
        <div className="campo">
          <label>Activo:</label>
          <input
            type="checkbox"
            checked={activo}
            onChange={e => setActivo(e.target.checked)}
          />
        </div>
        <button type="submit" className="boton-neon">Actualizar</button>
      </form>
      {mensaje && <p className="mensaje-feedback">{mensaje}</p>}
    </div>
  );
};

export default ActualizarTipoCatalogo;